...
import { SensorsReportType, selectEbReports, selectEbStatusApi, selectEndDate, selectGranularity, selectSensorLiveData, selectSensorReports, selectStartDate, selectStatusReportApi, selectTableColumns, selectTableDatas } from "src/store/slices/analizeSlice";
...
const EbChart: React.FC<Props> = (props) => {
	const chartOption = useAppSelector(selectChartOptions)
	// const chartRef = useRef<any>(null);
	const userData = useAppSelector(selectOwnUser)
	const selectDataOFChart = useAppSelector(selectEbReports)
	const statusReportApi = useAppSelector(selectEbStatusApi)
	...
	async function getdata(chartsettings?: ChartSettingsType) {
		...
		if (selectDataOFChart?.length) {
			...
			chartData.ebitem = [...props?.ebNames ?? []]
			if (selectDataOFChart?.[0]?.data !== undefined)
				chartData.sumOfdataEB(selectDataOFChart?.[0]?.data).then((data) => {
					console.log(data)
					setState(data)
	...
	return <section className={`flex justify-center mt-16 flex-wrap w-full `}>
		<Modal
			open={settingsModal}
